...
class message{
	...
	public function getConvas(){
		...
		while($row=mysqli_fetch_array($query)){
			...
			if(!in_array($user_to_push,$convas)){
				array_push($convas,$user_to_push);
			}
		}

		foreach ($convas as $username) {
			# code...
	...
	public function getConvasDropdown($data,$limit){
		$page=$data['page'];
		...
		if($count>$limit)
			...
			$return_string.="<input type='hidden' class='noMoreDropdownData' value='true'><p style='text-align:center;margin-top:15px;'>No more messages to load</p>";
